...
const TaxiCard = ({ image, model, gear, capacity, fuel, price8hr, price24hr, priceInfinity, icon }) => {
  ...
  const handleBooking = () => {
    ...
    let message = "";
    if (selectedPlan === "8 hr") message = "299km free, extra ₹8 per km";
    else if (selectedPlan === "24 hr") message = "259km free, extra ₹8 per km";
    else if (selectedPlan === "Infinity") message = "No extra charge";
  
    if (user) {
      // User is logged in, proceed to booking
      navigate("/selfdrivecar", {
        state: { model, selectedPlan, amount, message, formType: "car" },
      });
    } else {
      // Store booking details before redirecting to login
      sessionStorage.setItem("pendingBooking", JSON.stringify({ model, selectedPlan, amount, message, formType: "car" }));
      navigate("/signin");
    }
  };
